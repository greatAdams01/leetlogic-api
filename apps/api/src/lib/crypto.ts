...
function totpEncryptionKey(): Buffer {
  const configured = getConfig().TOTP_ENCRYPTION_KEY;
  return /^[a-f\d]{64}$/i.test(configured) ? Buffer.from(configured, "hex") : Buffer.from(configured, "base64");
}
...
export function decryptSecret(value: string): string {
  ...
  const decipher = createDecipheriv("aes-256-gcm", totpEncryptionKey(), Buffer.from(iv, "base64url"));
